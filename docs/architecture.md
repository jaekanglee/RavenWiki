...
* **D11: 그래프 레이아웃을 atlas(force-directed) 단일화 + 밀도 튠업**
  * 문제: D10 이후에도 그래프가 "원/트리 모양은 맞는데 어수선해 보인다"는 피드백이 이어졌다. 원인은 `_hierarchical_layout`/`_radial_hierarchical_layout`이 노드 위치를 **폴더 구조(깊이)로만** 정하고 실제 위키링크(엣지)는 위치 계산에 안 써서, 엣지가 레이아웃과 무관하게 화면을 가로지르는 구조적 불일치였다. `_hierarchical_layout`은 레이어를 고정폭(-420~420)에 나눠 담아 레이어당 노드가 많을수록(실측 depth3: 29개 → 간격 35.7) 무조건 더 뭉쳤고, radial도 "같은 깊이 = 같은 반지름"이라 근본은 동일 — 다만 원둘레가 반지름에 비례해서 커지는 우연 덕에 이 vault(깊을수록 노드가 많은 구조)에선 덜 뭉쳐 보였을 뿐, 얕은 depth에 노드가 몰리는 구조였다면 오히려 더 나빴을 것.
  * 조치: 실제 관계(엣지)로 위치를 정하는 atlas(ForceAtlas2/LinLog hybrid, `_forceatlas_layout`)만 남기고 `_hierarchical_layout`/`_radial_hierarchical_layout`/`_spring_layout`과 이들을 고르는 `layout` 쿼리 파라미터/드롭다운 UI를 전부 제거했다 (`_constellation_layout`은 atlas의 seed 좌표 생성에 내부적으로 계속 쓰여 존치). 지식그래프 도구(Obsidian/Roam/Logseq)들이 공통적으로 force-directed를 기본으로 쓰는 것과 같은 이유 — 위치가 곧 관계를 반영해야 위치와 엣지가 어긋나지 않는다.
