...
- **Dashboard 관리/설정 페이지(`VaultManage.tsx`) 내 PC REST API & MCP 엔드포인트 Tailscale IP 최우선 표시 개선**:
  - 내 PC에 Tailscale이 설치/구동 중일 경우, 로컬 REST API 및 MCP 카드의 대표 주소를 `127.0.0.1` 대신 감지된 **Tailscale IP 주소 (`http://100.x.y.z:8765` / `/mcp`)를 1순위로 표시**하도록 보강.
  - Tailscale 미구동 환경일 경우에만 폴백으로 `http://127.0.0.1:8765`를 표시하여 직관적인 연결 주소 복사 지원.
