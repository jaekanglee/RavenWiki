...
- **Dashboard 관리/설정 페이지(`VaultManage.tsx`) 서버 & API / MCP 환경 정보 카드 신설**:
  - REST API 엔드포인트 URL, MCP (LLM 에이전트) 엔드포인트 URL, 네트워크 바인딩 상태(`0.0.0.0`), CORS 활성화 상태를 한눈에 볼 수 있는 UI 카드 섹션 추가.
  - Claude Code, Cursor, Hermes 등 외부 LLM 클라이언트 설정 시 즉시 붙여넣어 활용할 수 있도록 `[📋 API URL 복사]`, `[📋 MCP URL 복사]` 원클릭 클립보드 복사 피드백 기능 탑재.
