# raven v0.5.0 — 카파시 LLM Wiki 운영정책 도입
...
**log.md (vault 루트) 자동화** + **raven-policy.md 템플릿** + **meta sync --with-log** + **CLI/API log endpoints** + **lint log_size check**.
...
### `raven.core.log` (60 LOC + 100 LOC test)
...
| `templates/raven-policy.md` | vault 루트 `raven-policy.md` (신규) | 카파시 가이드 통합 1페이지 (3-Layer + 5규칙 + 12 lint) |

→ `raven vault create` (bootstrap) 시 자동 복사.
→ 기존 vault 보강: `raven meta sync --with-log` (v0.5.0+ 옵션, 기존 파일 skip).
...
| 명령 | 역할 |
|---|---|
| `raven log list` | entry 조회 (--tail, --action, --json) |
| `raven log show` | raw 표시 (--limit, grep-style) |
| `raven log append` | 수동 추가 (--action, --files, --note) |
| `raven log rotate` | log-YYYY.md로 rotate (--year, --force) |
| `raven log status` | entries 수 + last + needs_rotate (--json) |

```bash
raven log list --tail 10
raven log list --action build --json
raven log append "manual note" --action chore --note "context"
raven log status
```
...
`raven.core.lint.check_log_size(vault)`:
...
→ `run_lint()` 결과에 `log_issues` 필드 추가. `raven build` 시 자동 호출.
...
| 파일 | 종류 | LOC |
|---|---|---|
| `raven/core/log.py` | **신규** | 320 |
| `raven/core/lint.py` | 수정 | +60 (log_size check) |
| `raven/core/vault.py` | 수정 | +50 (bootstrap + sync_meta --with-log) |
| `raven/core/db.py` | 수정 | +20 (log hook) |
| `raven/core/__init__.py` | 수정 | +2 (log_module export) |
| `raven/cli/__main__.py` | 수정 | +150 (log_app + 5 commands + meta sync --with-log) |
| `raven/api/server.py` | 수정 | +90 (4 endpoints + LogAppend model) |
| `raven/core/templates/SCHEMA.md` | 수정 | +60 (정책 강화) |
| `raven/core/templates/log.md` | **신규** | 8 |
| `raven/core/templates/raven-policy.md` | **신규** | 80 |
...
| `~/.hermes/.../raven/SKILL.md` | 수정 | +30 (log 섹션 + 다음 단계) |
...
```bash
raven vault create <name> <path>   # → SCHEMA/RULES/log/policy 자동 복사
```

### 기존 vault 보강 (v0.5.0 도입 시)

```bash
raven meta sync --with-log --vault <name>
# → log.md + raven-policy.md 추가 (없던 vault에 한함)
```
