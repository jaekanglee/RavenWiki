...
### 1-4. 에이전트 지침 템플릿의 LLM Wiki 사상 전면 주입 및 정제
* **`raven/core/templates/agent/README.md`**:
  * 새로운 지식을 작성(Write)하기 전에 이미 컴파일된 지식이 없는지 적극적으로 검색(Read)하여 재사용하게 하는 **"Read-heavy, Write-rare" (지식 저비용 재사용)** 규칙을 공식 가이드로 앵커링했습니다.
* **`raven/core/templates/agent/SAFETY.md`**:
  * 에이전트가 큐레이션을 하면서 독단적으로 문서를 대량 삭제하거나 병합하여 지식을 소실하는 행위를 방지하기 위해 **"사용자 승인 없는 파괴적 큐레이션 금지"**를 강한 금지령(#9)으로 명문화했습니다.
  * Path Traversal 섹션의 깨진 빈 테이블 서식을 보완하여 명확히 정돈했습니다.
* **`raven/core/templates/agent/WORKFLOW.md`**:
  * **"정적 린터(도구: Raven) ↔ 동적 판단자(에이전트)"**의 역할 경계를 정립했습니다. Raven은 멍청하고 기계적인 린트(데드링크, 포맷 에러)만 정적으로 진단하고, 에이전트가 이를 해석하여 동적인 병합/아카이브/모순해결 판단을 내려 [Curation Proposal]을 작성해 사용자의 컨펌을 받도록 워크플로우를 고도화했습니다.
