...
def test_load_parses_entries(vault):
    ...
    assert entries[0].subject == "page-a"
    assert "reason: first" in entries[0].details
    assert entries[1].action == "update"
    assert "files: [content/b]" in entries[1].details
...
def test_format_grep_compatible(vault):
    ...
    assert len(grep_style) >= 2  # placeholder + 2 actual
