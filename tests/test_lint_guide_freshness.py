...
def test_lint_19_no_agents_dir(tmp_path):
    """`_meta/agents/` 부재 — info 2건 (SCHEMA + RAVEN-CONTRACT 부재)."""
...
def test_lint_19_stamp_fresh(tmp_path):
    ...
    contract_hash = _sha256(vault.root / "_meta" / "agents" / "RAVEN-CONTRACT.md")
    _write_stamp(vault.root, {"SCHEMA": schema_hash, "RAVEN-CONTRACT": contract_hash})
    issues = check_guide_freshness(vault)
    assert issues == []
...
def test_lint_19_stamp_stale(tmp_path):
    ...
    _write_agents(vault.root)
    # stamp은 옛 hash로 박음
    _write_stamp(vault.root, {"SCHEMA": "stale_sha", "RAVEN-CONTRACT": "stale_contract"})
    issues = check_guide_freshness(vault)
    assert len(issues) == 2
