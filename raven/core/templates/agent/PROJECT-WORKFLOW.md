...
- **저널 요약 강제**: 일지(`journal`) 및 저널 성격의 문서를 작성할 때는 본문 최상단에 **반드시 3줄 이내의 `# 요약` 섹션을 명확히 작성**해야 합니다. 기계적인 태스크 코드나 빌드 메시지에 의존한 서술은 금지됩니다. (감지 시 Lint #22 warning)
- **플레이스홀더 및 무의미한 문구 금지**: 본문이나 frontmatter 내에 `TBD`, `N/A`, `추후 작성` 등 의미 없는 플레이스홀더를 작성하지 않습니다. 작성할 내용이 부족한 섹션은 섹션 자체를 삭제합니다. (감지 시 Lint #20 critical 에러)
- **본문에 actor/run_id/tool/idempotency_key 같은 운영 메타 노출 금지**
- **헤더는 순수 자연어** (`## 결론`, 영문 괄호 병기 금지)
- **맥락을 동반한 위키링크**: 단순한 링크 삽입을 지양하고, `[[slug]] — {연결 관계 설명}` 포맷으로 적어 RAG 탐색 시 링크의 상관관계를 쉽게 파악할 수 있도록 돕습니다. (설명 생략 시 Lint #21 warning)
