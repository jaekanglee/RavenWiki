...
> **핵심 원칙**: 쓰고 나서 30초 안에 핵심을 파악할 수 없으면 너무 많이 쓴 것.
> 사람이 읽기 좋게, 핵심만, 명료하게. 분량이 많다고 잘 쓴 것이 아니다.

### 형식

- **BLUF**: 페이지 첫 줄에 결론/결정 1문장
- frontmatter는 구조화, 본문은 자연스러운 문장으로 작성
- 필수 섹션 최소화 (`요약` / `내용` / `관련` 정도), 내용 없는 섹션은 **삭제**
- **헤더는 순수 자연어** (`## 결론`, 영문 괄호 병기 금지)
- **슬러그 번역/음차 금지**: 파일명(slug)은 frontmatter `title`을 그대로 슬러그화 (공백·특수문자 → `-`, 영문 소문자). 한글 title → 영문/로마자 변환 ❌

### 분량·밀도

- **journal 기준 50줄 이내 지향**. 100줄 초과 시 핵심만 남기고 상세 내용은 `raw/`로 분리
- **섹션 중복 금지**: 같은 정보를 다른 헤딩으로 반복 ❌ (예: `# 요약`과 `## Summary` 동시 존재 ❌)
- **운영 메타 본문 노출 금지**: `actor` / `run_id` / `tool` / `idempotency_key` / commit hash 나열 / test count 나열 등 기계적 수치는 핵심 판단이 필요한 경우만 1줄로
- **본문·frontmatter에 운영 에이전트 정보 노출 금지**: `agents: name: ... timestamp: ...` ❌

### 요약 퀄리티

- **저널 `# 요약` 강제**: 본문 최상단, 3줄 이내. "무엇을 했고, 결과가 무엇인가"를 사람이 바로 파악할 수 있어야 함
- commit hash · test count · 빌드 수치 나열은 요약이 아님. 판단·결과·임팩트 중심으로
- **플레이스홀더 금지**: `TBD` / `N/A` / `추후 작성` 등 — 내용 없는 섹션은 섹션 자체 삭제 (Lint #20 critical)

### self-audit 섹션

- **모두 통과(✅)인 경우 섹션 생략** — 잡음이 됨
- 실질적인 문제나 교훈이 있을 때만 기록. 항상 붙이는 체크리스트 ❌

### 위키링크

- **맥락 동반 필수**: `[[slug]] — {연결 관계 설명}` 포맷 (Lint #21 warning)
- **존재하지 않는 slug 링크 금지**: 발행 전 대상 페이지 존재 확인. 미확인 링크는 텍스트로만 (Lint #1 critical)
- vault 외부 경로(`host plans/...`, `.hermes/...`)는 wikilink 형식 사용 ❌ — 텍스트 참조로만
...
11. **#20 플레이스홀더 발견** — 단순히 TBD 문구만 지우지 않고, 원본/인접 문서의 정보로 내용을 보완하여 완성하거나, 정보가 없는 미작성 영역인 경우 해당 빈 섹션/문단을 완전히 도려내어(클렌징) RAG 임베딩 노이즈를 제거한다.
12. **#21 맥락 없는 링크 발견** — 단순히 `[[link]]`만 나열된 곳에 Target 문서와의 개념적 교차점과 참고 이유를 명시하는 `— {연결 맥락}` 꼬리표 설명을 한 줄 덧붙여 큐레이팅한다.
13. **#22 저널/이슈 요약 누적** — CLI 빌드 아웃풋이나 수백 줄의 에러 로그가 복사-붙여넣기되어 지면을 낭비하는 경우, 핵심 에러 메시지 1줄과 상위 3줄 스택 트레이스로 요약·압축하고 원본 로그는 `raw/logs/`에 파일로 분리 이주시켜 RAG 노이즈를 클렌징한다.
14. 점검 결과가 §3 저장 신호를 통과하면 journal로 기록
...
### §7.0 대화 중 자동 감지·발행 원칙 (v0.7.120+)

> **"사람이 '발행해'라고 말하기 전에 에이전트가 먼저 감지하고 제안해야 한다."**

에이전트는 대화 흐름 중 아래 신호를 감지하면 **발행 여부를 먼저 제안**한다.
사용자 명시 명령을 기다리지 말 것.

| 감지 신호 | 발행 후보 type | 행동 |
|---|---|---|
| 구현/작업 완료 보고 (commit, 기능 완성, 배포 등) | `journal` | "vault에 기록할게요?" 제안 |
| 방향 결정, 정책 확정, 트레이드오프 선택 | `decision` (draft) | "결정 페이지 발행할게요?" 제안 |
| 새 개념·용어 정의, 아키텍처 설명 | `concept` (draft) | "concept 페이지 만들게요?" 제안 |
| 반복되는 문제 패턴, lesson learned | `journal` 내 Lessons 섹션 또는 별도 `concept` | 포함 제안 |
| 이슈 발견, 버그 리포트, 미해결 항목 | `issue` (status=draft) | 자율 발행 가능 (§7.1) |

**제안 방식**: 한 줄로 명확하게.
> 예: "이 결정 vault에 decision 페이지로 남겨두면 다음 세션에서 바로 이어갈 수 있어요. 발행할게요?"

**발행 금지 상황**: §3 저장 신호 4가지 모두 미통과 시 — 제안도 하지 않는다.

### §7.0.1 자동 발행이 안 되는 원인 자가점검

에이전트가 대화 중 발행을 놓쳤다면:
1. §3 신호 중 하나라도 해당하는가? → 해당하면 발행 제안이 맞음
2. 사용자가 명시 명령을 내릴 때까지 기다리고 있었는가? → 기다리는 것이 잘못
3. 다음 세션에서 이 내용을 바로 이어받을 수 있는 상태인가? → 아니면 발행해야 함
