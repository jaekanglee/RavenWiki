...
발행자는 본문을 작성한 뒤에만 연결을 생각하지 않습니다. 소스 검증과 함께 아래를 처리합니다.

| 대상 | 발행자 행동 | 금지 경계 |
|---|---|---|
| `sources`·`confidence` | 실제로 사용한 소스와 검증 수준을 기록 | 읽지 않은 문서·추정 출처를 기록하지 않음 |
| 본문 `[[wikilink]]` | 독자가 다음에 확인할 문서를 맥락 설명과 함께 연결 | 링크 수를 맞추거나 무맥락 목록을 만들지 않음 |
| `relations` | `uses`/`depends_on`/`implements`/`implemented_by`/`related` 중 확인된 의미를 선택하고 `evidence`·`reason`을 기록 | 추천·태그 유사도만으로 관계를 확정하지 않음 |
| `aliases`·충돌·상태 | rename/병합, 조사된 충돌, 권한 있는 상태 전이에서만 설정 | 편의상 별칭·모순·archive를 임의 설정하지 않음 |

대시보드의 "함께 읽어볼 만한 문서"는 탐색 후보입니다. 후보를 관계로 바꾸려면 소스와 본문을 다시 확인하고 관계 유형과 이유를 명시해야 합니다.
