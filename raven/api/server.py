...
def vault_graph(
    ...
    if wiki_db.exists():
        try:
            ...
            pages = db.execute("SELECT slug, title, type, importance, centrality, community FROM pages").fetchall()
...
def get_vault_advice(name: str):
    ...
    try:
        ...
        relations = db.execute("SELECT source_slug, target_slug FROM relations").fetchall()
